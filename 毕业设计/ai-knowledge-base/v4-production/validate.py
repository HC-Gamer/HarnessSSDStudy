...
# ---------------------------------------------------------------------------
# V3 段间校验（KBState 形态）—— 挂在 graph 的 collect / analyze 之后做闸门
# ---------------------------------------------------------------------------
#
# 与上面几个 validator 的区别：上面校验的是「落盘 JSON 文件」，这里校验的是
# 「LangGraph 节点之间流动的 state」。职责一致（结构对不对），载体不同。
#
# 设计边界：段间校验只管**结构合法性**，不管**内容好不好** —— 内容质量是
# Reviewer 的活。所以「摘要太短」是警告不是错误，否则低质产出会在进入
# 审核循环之前就被打死，revise 回路永远没机会跑。

#: V3 collector 的合法采集模式
V3_COLLECTION_MODES: frozenset[str] = frozenset({"github", "rss", "degraded"})

#: V3 analyzer 的合法分类（与 workflows/analyzer.py VALID_CATEGORIES 保持一致）
V3_CATEGORIES: frozenset[str] = frozenset(
    {"llm", "agent", "rag", "multimodal", "tools", "industry"}
)

#: V3 摘要字数下限（低于只警告，交给 Reviewer 打分）
V3_MIN_SUMMARY_CHARS = 60


def validate_sources_segment(state: dict[str, Any]) -> ValidationResult:
    """校验 ``collect`` 节点交给 ``analyze`` 的 ``sources``。

    硬错误：``sources`` 非 list / 为空、条目非 dict、``title`` 缺失或为空、
    ``url`` 不是 http(s)、``stars`` 为负。

    警告：``description`` 为空、``collection_mode`` 是 ``degraded``
    （能跑通但产出不可引用，下游要知情）。

    Args:
        state: 当前 KBState（当 dict 用）。

    Returns:
        ``(passed, errors, warnings)``。

    Examples:
        >>> ok, _, _ = validate_sources_segment({"sources": []})
        >>> ok
        False
        >>> ok, _, _ = validate_sources_segment({"sources": [
        ...     {"title": "a/b", "url": "https://x.com", "description": "d",
    ...
    if not isinstance(sources, list):
        ...
        return bag.result()
    if not sources:
        bag.error("sources 为空 —— 采集段没交出任何内容，不能让 analyze 凭空发挥")
        return bag.result()

    for index, item in enumerate(sources):
        sub = bag.child(f"sources[{index}] ")
        if not isinstance(item, dict):
            sub.error(f"类型错误: 期望 dict，实际 {type(item).__name__}")
            continue

        title = item.get("title")
        if not isinstance(title, str) or not title.strip():
            sub.error("title 缺失或为空")

        url = item.get("url")
        if not isinstance(url, str) or not URL_PATTERN.match(url):
            sub.error(f"url 格式错误: {url!r}，应以 http:// 或 https:// 开头")

        stars = item.get("stars", 0)
        if not isinstance(stars, int) or stars < 0:
            sub.error(f"stars 非法: {stars!r}，应为非负整数")

        description = item.get("description", "")
        if not isinstance(description, str) or not description.strip():
            sub.warn("description 为空，分析质量会受影响")

        mode = item.get("collection_mode")
        if mode is not None and mode not in V3_COLLECTION_MODES:
            sub.error(f"collection_mode 非法: {mode!r}，可选 {sorted(V3_COLLECTION_MODES)}")
        elif mode == "degraded":
            sub.warn("collection_mode=degraded：真实采集全部失败，产出不可引用")

    return bag.result()


def validate_analyses_segment(state: dict[str, Any]) -> ValidationResult:
    """校验 ``analyze`` 节点交给 ``review`` 的 ``analyses``。

    硬错误：``analyses`` 非 list / 为空、条目非 dict、``summary`` 非 str /
    为空、``relevance_score`` 非数值或不在 0–1、``tags`` 非 list。

    警告：摘要短于 :data:`V3_MIN_SUMMARY_CHARS`、``category`` 不在白名单、
    ``tags`` 为空。
    ...
    Examples:
        >>> ok, errs, _ = validate_analyses_segment({"analyses": [
        ...     {"summary": "x" * 80, "relevance_score": 1.5, "tags": []}]})
        >>> ok
        False
    """
    bag = _Collector()
    ...
    if not isinstance(analyses, list):
        bag.error(f"analyses 类型错误: 期望 list，实际 {type(analyses).__name__}")
        return bag.result()
    if not analyses:
        bag.error("analyses 为空 —— 分析段没有产出，不能静默传给审核")
        return bag.result()

    for index, item in enumerate(analyses):
        sub = bag.child(f"analyses[{index}] ")
        if not isinstance(item, dict):
            sub.error(f"类型错误: 期望 dict，实际 {type(item).__name__}")
            continue
        ...
        if not isinstance(summary, str):
            sub.error(f"summary 类型错误: 期望 str，实际 {type(summary).__name__}")
        elif not summary.strip():
            sub.error("summary 为空 —— 分析节点没产出内容")
        elif _chars(summary) < V3_MIN_SUMMARY_CHARS:
            sub.warn(f"summary 只有 {_chars(summary)} 字，低于 {V3_MIN_SUMMARY_CHARS} 字")

        score = item.get("relevance_score")
        if not isinstance(score, (int, float)) or isinstance(score, bool):
            sub.error(f"relevance_score 类型错误: {score!r}，应为 0-1 之间的数值")
        elif not 0.0 <= float(score) <= 1.0:
            sub.error(f"relevance_score 越界: {score}，量纲应为 0-1（见 PLANNING §8.2）")

        tags = item.get("tags")
        if not isinstance(tags, list):
            sub.error(f"tags 类型错误: 期望 list，实际 {type(tags).__name__}")
        else:
            _check_str_list(tags, "tags", sub)
            if not tags:
                sub.warn("tags 为空，Wk4 的检索与分类会退化")

        category = item.get("category")
        if category is not None and category not in V3_CATEGORIES:
            sub.warn(f"category={category!r} 不在白名单 {sorted(V3_CATEGORIES)}")

    return bag.result()


#: V3 段名 → 校验函数（graph.py 的闸门直接查这张表）
V3_SEGMENT_VALIDATORS = {
    "sources": validate_sources_segment,
    "analyses": validate_analyses_segment,
}
